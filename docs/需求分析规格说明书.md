...
| **主成功场景** | 1. 管理员打开导入页面2. 选择一个或多个 PDF/MD 文件3. 系统返回 `task_ids`4. SSE 推送各节点进度5. 全部节点完成，状态变为 `completed` |
| **扩展场景**  | 3a. MinIO 上传失败：记录警告，继续本地处理5a. 某节点异常：状态变为 `failed`，SSE 推送 `error`                           |
...
| **主成功场景** | 1. 用户输入自然语言问题2. 系统确认商品型号3. 多路检索 + RRF + 重排4. 流式返回答案与来源5. 会话写入 MongoDB |
| **扩展场景**  | 2a. 型号不明确：直接反问，跳过检索2b. 无匹配商品：拒绝回答并提示4a. 非流式模式：一次性返回完整 JSON            |
| **后置条件**  | 用户获得回答；会话历史更新                                                         |
...
| 原始文件          | MinIO + 本地 `output/`       | 按日期 / task\_id 分层                  |
| 会话消息          | MongoDB                    | 库名 `kb`                            |
...
| AC-07 | 宿主机可通过 VM\_IP 连接全部中间件 | `.env` 配置 VM IP 后服务正常启动             |
...
| 版本   | 日期         | 变更内容          | 变更人 |
| ---- | ---------- | ------------- | --- |
| V1.0 | 2026-03-06 | 初稿，依据现有代码逆向整理 | 李梓楠 |
