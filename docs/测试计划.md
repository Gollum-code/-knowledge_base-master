...
| 层次   | 占比（工作量） | 执行人     | 说明           |
| ---- | ------- | ------- | ------------ |
| 单元测试 | 20%     | 王梓瑜     | 关键节点与工具函数    |
| 接口测试 | 35%     | 王梓瑜     | Postman 集合为主 |
| 系统测试 | 30%     | 王梓瑜     | 前后端 + 中间件联调  |
| 验收测试 | 15%     | 王梓瑜、李梓楠 | 对照需求验收标准     |
...
| 用例编号   | 用例名称          | 工具             | 步骤                                                     | 预期结果                           | 优先级 |
| ------ | ------------- | -------------- | ------------------------------------------------------ | ------------------------------ | --- |
| IMP-01 | 上传 PDF        | Postman        | POST `/upload`，Body→form-data，`files`=test\_sample.pdf | 200，`task_ids` 非空              | P0  |
| IMP-02 | 上传 MD         | Postman        | POST `/upload`，`files`=test\_sample.md                 | 200，返回 task\_id                | P0  |
...
| QRY-03 | 流式查询       | Postman + curl | POST `/query`，`is_stream=true`，再 SSE | 先返回 session\_id，SSE 有 delta/final | P0  |
...
| QRY-09 | 多轮对话       | Postman ×2     | 同一 session\_id 连续提问                  | 第二次可引用上下文                         | P1  |
...
| 用例编号   | 用例名称     | 工具           | 指标            | 优先级            | <br /> |
| ------ | -------- | ------------ | ------------- | -------------- | :----- |
| NFR-01 | 上传接口响应   | Postman      | ≤ 3s（不含后台处理）  | P1             | <br /> |
| NFR-02 | 健康检查响应   | Postman      | ≤ 500ms       | P2             | <br /> |
| NFR-03 | 同步问答耗时   | Postman      | ≤ 30s         | P1             | <br /> |
| NFR-04 | 日志写入     | 文件系统         | `logs/` 有按日日志 | P2             | <br /> |
...
| 验收项             | 对应用例          | 工具              | 通过标准             |
| --------------- | ------------- | --------------- | ---------------- |
| AC-01 PDF 导入入库  | IMP-01、IMP-06 | Postman、Attu    | kb\_chunks 有数据   |
| AC-02 MD 跳过 PDF | IMP-02、IMP-07 | Postman、SSE     | 无 pdf\_to\_md 节点 |
...
| 阶段   | 日期          | 内容                              | 负责人     | 产出                       |
| ---- | ----------- | ------------------------------- | ------- | ------------------------ |
| 测试准备 | 05-25       | 搭建环境、准备 Postman Collection、测试数据 | 王梓瑜     | Collection + Environment |
| 接口测试 | 05-26～05-27 | 执行 IMP / QRY 用例                 | 王梓瑜     | 接口测试报告                   |
| 系统测试 | 05-28～05-29 | 前后端联调、Attu/Compass 验证           | 王梓瑜     | 缺陷清单                     |
| 回归测试 | 05-30       | Bug 修复后 Collection Runner 重跑 P0 | 王梓瑜     | 回归记录                     |
| 验收测试 | 05-31       | 对照 AC-01～08 逐项验收                | 王梓瑜、李梓楠 | 验收测试报告                   |
